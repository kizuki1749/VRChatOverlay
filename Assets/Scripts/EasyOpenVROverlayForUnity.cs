...
{
    [SerializeField]
    string[] ExclusionNames = new string[] { "Text", "Checkmark", "Label", "Background", "Panel" };
    [SerializeField]
...
#pragma warning restore 0219
        ...
        if (((results.fDistance < TapOnDistance && PlayerPrefs.GetInt("NeedButtonPush", 1) == 0) || (PlayerPrefs.GetInt("NeedButtonPush", 1) == 1 && ulong.Parse(PlayerPrefs.GetString("ControlButton", "4")) == MenuSystemUtil.GetButton(util, ControllerHand.Any))) && !tapped)
        {
            if (PlayerPrefs.GetInt("NeedButtonPush", 1) == 1)
            {
                hand = MenuSystemUtil.GetControllerHand(util);
        ...
        if (((results.fDistance > TapOffDistance && PlayerPrefs.GetInt("NeedButtonPush", 1) == 0) || (PlayerPrefs.GetInt("NeedButtonPush", 1) == 1 && MenuSystemUtil.GetButton(util, ControllerHand.Any) == 0)) && tapped)
        {
            if (PlayerPrefs.GetInt("NeedButtonPush", 1) == 1)
            {
                if (hand == ControllerHand.Right)
...
#pragma warning restore 0219
        ...
        for (int i = 0; i < result.Count; i++)
        {
            ...
            bool flag = true;
            foreach (string text in ExclusionNames)
            {
                if (res.gameObject.name.Contains(text))
                {
                    flag = false;
                    break;
                }
            ...
            if (res.isValid && flag)
            {
                ...
                {
                    if (PlayerPrefs.GetInt("NeedButtonPush", 1) == 0 || MenuSystemUtil.GetButton(null, ControllerHand.Any) == ulong.Parse(PlayerPrefs.GetString("ControlButton", "4")))
